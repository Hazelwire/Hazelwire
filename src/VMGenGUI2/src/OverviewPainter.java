...
public class OverviewPainter implements PaintListener, Observer, MouseListener {
		...
		public void fillHeight(){
			...
			int modsHeight = (1 + ModsBookkeeper.getInstance().getSelectedMods().size()) * 15;
			if(!sameSize && modsHeight< canvas.getParent().getSize().y){
				canvas.setSize(canvas.getBounds().width, canvas.getParent().getBounds().height - 4);
			}
		}
		...
		public void paintControl(PaintEvent p) {
			...
			for(i = 15; i <= height; i=i+15){
				...
				if(i == 15){
					...
					g.drawText("Challenge", x + (7*width/15)+15 -3, y+i-15);
					g.drawText("Points", x + (10*width/15)+15 -3, y+i-15);
					g.drawText("Subtotal", x + (12*width/15)+15 -3, y+i-15);
				}
					...
					if(printThis.get((i/15)-2) instanceof Mod){
						...
						g.drawText(mod.getPoints()+"", x + (12*width/15)+15, y+i-15);
					}
					else if(printThis.get((i/15)-2) instanceof Challenge){
						Challenge c = (Challenge)printThis.get((i/15)-2);
						g.drawText(c.getId(), x + (7*width/15)+15, y+i-15);
						g.drawText(c.getPoints()+"", x + (10*width/15)+15, y+i-15);
					}										
				}
			}
			if(i%2 == 0){
			...
			g.setLineWidth(2);
			g.drawLine(x+ (7*width/15+3), y, x+ (7*width/15+3), y+height+3);
			g.drawLine(x+ (10*width/15+3), y, x+ (10*width/15+3), y+height);
			g.drawLine(x+ (12*width/15+5), y, x+ (12*width/15+5), y+height);
